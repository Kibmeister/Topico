...
function choose () {
  ...
  WordsClass.getQueWords(chosenWord.word, function (err, res) {
    ...
    wordQuewords.push({ word: res[0].queword1, lcd: fourWords[(wordIndex) % 4].lcd })
    console.log('Pushed entry: ', (wordIndex + 1) % 4)
    wordQuewords.push({ word: res[0].queword2, lcd: fourWords[(wordIndex + 1) % 4].lcd })
    console.log('Pushed entry: ', (wordIndex + 2) % 4)
    wordQuewords.push({ word: res[0].queword3, lcd: fourWords[(wordIndex + 2) % 4].lcd })
